...
本文档提供了扩展词汇数据库的详细说明，包括单词对象结构、难度分配、SVG 生成和测试程序。
...
4. [将单词添加到 words.js](#将单词添加到-wordsjs)
5. [测试新单词](#测试新单词)
...
| 级别 | 描述 | 特征 | 示例 |
|------|------|------|------|
| ⭐ 1 | 初学者 | 非常常见、短、具体 | apple, run, big |
| ⭐⭐ 2 | 基础 | 常见、简单结构 | happy, quickly, because |
...
#### 1. 单词长度（40% 权重）
- **1-3 字母**：分数 1（例如："run"、"big"、"cat"）
- **4-5 字母**：分数 2（例如："apple"、"happy"）
- **6-7 字母**：分数 3（例如："beautiful"、"quickly"）
- **8-10 字母**：分数 4（例如："wonderful"、"carefully"）
- **11+ 字母**：分数 5（例如："magnificent"、"unfortunately"）

#### 2. 音节数（35% 权重）
- **1 音节**：分数 1（例如："run"、"big"）
- **2 音节**：分数 2（例如："ap-ple"、"hap-py"）
- **3 音节**：分数 3（例如："beau-ti-ful"）
- **4+ 音节**：分数 4-5（例如："mag-nif-i-cent"）
...
2. **考虑学习环境**：早期教授的单词应该更简单
3. **评估抽象度**：是否容易可视化？
4. **与类似单词比较**：保持一致性

**调整示例**：
- "love"（4 字母，1 音节）→ 自动：1，手动：2（抽象概念）
- "the"（3 字母，1 音节）→ 自动：1，保持：1（最常见单词）
- "although"（8 字母，2 音节）→ 自动：3，手动：3（正确）
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 单词不出现 | words.js 中的语法错误 | 检查控制台错误，验证 JSON |
| SVG 不显示 | 无效的 SVG 标记 | 使用在线验证器验证 SVG |
| 错误的难度 | 自动计算偏差 | 手动调整难度值 |
| 卡片布局损坏 | 缺少必需字段 | 添加所有必需字段 |
| TTS 不工作 | 单词中的特殊字符 | 使用标准英文字符 |
...
为了获得更好的质量或批量生成，请使用提供的提示模板使用外部 AI 服务。

#### 推荐服务

| 服务 | 质量 | 速度 | 费用 | 最适合 |
|------|------|------|------|--------|
| ChatGPT (GPT-4) | 优秀 | 快 | 付费 | 高质量单个单词 |
| Claude (Anthropic) | 优秀 | 快 | 付费 | 批量生成 |
| DeepSeek | 良好 | 快 | 便宜 | 经济实惠的批量 |
| Gemini (Google) | 良好 | 快 | 免费层 | 免费替代方案 |

#### AI 提示模板

查看 `ai-word-generation-prompt.md` 获取完整的提示模板。

**快速模板**：

```
为英文单词"[WORD]"生成一个完整的单词对象，遵循以下结构：
...
- 使用 IPA 标记表示发音
- 保持例句简单（10-20 个单词）
- SVG 应该是 200x200 viewBox，简单设计，2-3 种颜色
- 难度基于单词长度、音节和抽象度
- 标签应该是描述性和相关的
...
1. **提前计划**：创建包含所有单词数据的电子表格
2. **批量生成**：一次使用 AI 生成 10-20 个单词
3. **系统审查**：彻底检查每个单词
4. **增量测试**：以小批量（5-10 个单词）添加和测试
5. **频繁提交**：每批后进行 git 提交
...
### 发音
- [剑桥词典](https://dictionary.cambridge.org/) - IPA 发音
